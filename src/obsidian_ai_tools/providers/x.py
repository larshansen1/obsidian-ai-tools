...
(chrome-extension/capture.js). X blocks server-side scraping, so never hit
x.com from here.

Fallback: Supadata metadata endpoint (GET /v1/metadata) for URL-only ingests
(CLI / reading list). Verified against docs.supadata.ai 2026-09: there is no
...
from ..models import ThreadMetadata, ThreadTweet
