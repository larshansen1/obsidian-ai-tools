...
class _FixedDatetime:
    """Deterministic datetime.now() replacement for cache tests."""
...
class TestVideoCache:
    ...
    def test_cache_entry_returned_regardless_of_age(
        self, cache_dir: Path, sample_metadata: VideoMetadata
    ) -> None:
        """A cache entry is returned regardless of age."""
        cache = VideoCache(cache_dir)
        cache.set("test123", sample_metadata, "unofficial")
        result = cache.get("test123")
        assert result is not None
    ...
    def test_get_returns_entry_regardless_of_age(
        self,
        cache_dir: Path,
        sample_metadata: VideoMetadata,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """A cache entry is returned even well past TTL."""
        cache = VideoCache(cache_dir)
        fixed_now = datetime(2026, 1, 1, 12, 0, 0)
        cache.set("test123", sample_metadata, "unofficial")
        cache_file = cache.cache_dir / "test123.json"
        data = json.loads(cache_file.read_text())
        data["cached_at"] = (fixed_now).isoformat()
    ...
    def test_stats_counts_with_corrupted(
        self,
        cache_dir: Path,
        sample_metadata: VideoMetadata,
    ) -> None:
        """Stats count total and corrupted entries correctly."""
        cache = VideoCache(cache_dir)
        for vid in ["valid1", "valid2", "valid3"]:
